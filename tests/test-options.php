 ...
 *
 * WP-RelativeDate has nothing for a site owner to configure, so it stores no
 * settings row at all -- only the version markers. That absence is the point
 * worth pinning: it is a deliberate exemption (§2.1), not a row that failed to
 * be created, and a later change that quietly reintroduces an empty autoloaded
...
class WP_RelativeDate_Options_Test extends WP_RelativeDate_TestCase {
	...
	public function test_no_settings_row_is_ever_created() {
		...
		$this->assertFalse(
			...
			'WP-RelativeDate has no settings, so it must not store a settings row.'
		);
	}
	...
	public function test_the_class_exposes_no_settings_api() {
		$this->assertFalse(
			defined( 'WP_RelativeDate_Options::OPTION' ),
			'A settings row constant means a settings row is coming back.'
		);
		$this->assertFalse(
			method_exists( 'WP_RelativeDate_Options', 'sanitize' ),
	...
	public function test_markers_normalises_a_corrupt_row_without_writing() {
		...
		$this->assertSame( array(), WP_RelativeDate_Options::markers() );
	}
